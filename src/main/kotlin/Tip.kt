...
object Tip {
    ...
    fun processFile(file: File, options: RunOption) {
        ...
        val parserResult = tipParser.tryParseToEnd(program)
        if (parserResult !is Parsed) {
            log.error("Failure parsing the program: $file\n${parserResult}")
        } else {
            val programNode = parserResult.value as AProgram
            ...
            if (options.cfg || options.dfAnalysis.any { p -> p.value != dfo.Disabled && !p.value.interprocedural() }) {
                ...
                options.dfAnalysis.forEach {
                    if (!it.value.interprocedural()) {
                        ...
                        val res = an!!.analyze()
                        val res2 =
                            if (it.value.contextsensitive())
                                Output.transform(res as MapWithDefault<Pair<CallContext, CfgNode>, *>)
                            else res as Map<CfgNode, *>
                        Output.output(file, DataFlowOutput(it.key), wcfg.toDot({ node ->
                            Output.labeler(res2, node)
